-- 00009_rpc_confirm_purchase.sql
-- Confirmar compra (Secao 13.1).
--
-- Converter uma oportunidade em estoque toca quatro tabelas: watches, expenses,
-- financial_transactions e purchase_opportunities. Fazer isso em quatro
-- chamadas separadas do cliente permite que uma falhe no meio e deixe, por
-- exemplo, o relogio criado sem a saida no caixa.
--
-- Uma funcao plpgsql roda inteira em uma unica transacao: ou tudo acontece, ou
-- nada acontece.
...
begin
  ...
  values (
    ...
    'OWNED', 'AVAILABLE', p_supplier_id,
    coalesce(p_data_compra, current_date), p_observacoes
  )
  ...
  values (
    v_owner, 'EXPENSE', 'PURCHASE', p_valor_fechado, 'CONFIRMED',
  ...
  values (
    ...
    p_valor_fechado, coalesce(p_data_compra, current_date), 'CONFIRMED',
    v_transaction_id
  )
  ...
  set status = 'PURCHASED',
      ...
      data_fechamento = coalesce(p_data_compra, current_date),
...
/*
 * A assinatura vai explicita: a migration 00010 cria uma segunda versao desta
 * funcao, e `on function public.confirm_purchase` sem argumentos fica ambiguo
 * quando existe mais de uma sobrecarga (quebra ao reaplicar as migrations).
 */
